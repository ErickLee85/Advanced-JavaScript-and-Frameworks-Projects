...
import SoundFX from "../../sounds/click.mp3";

const myMusic : any = new Howl({
  src: [backgroundMusic]
});

var sfx: any = new Howl({
  src: [SoundFX]
});
...
export default class App extends React.PureComponent<Props, State> {
  ...
  PlayAudio = () => {
    if (myMusic.playing() === false) { //using this conditional to make sure sound doesnt repeat on top of its self
       myMusic.play()
    }
   }
  ...
  render() {
    return (
      ...
      <div className={styles.app} onClick={this.userClicks}>
        {this.renderBoard()}
